...
# model_LSTM_self = create_LSTM_self()

# 预测结果
# Predict('AMZN', AMZN_x_test, AMZN_y_test, model_CNN)
# Predict('TSLA', TSLA_x_test, TSLA_y_test, model_CNN)
# Predict('AMZN', AMZN_x_test, AMZN_y_test, model_RNN)
# Predict('GOOG', GOOG_x_test, GOOG_y_test, model_RNN)
# Predict('TSLA', TSLA_x_test, TSLA_y_test, model_RNN)
Predict('AMZN', AMZN_x_test, AMZN_y_test, model_LSTM)
# Predict('GOOG', GOOG_x_test, GOOG_y_test, model_LSTM)
# Predict('TSLA', TSLA_x_test, TSLA_y_test, model_LSTM)
# Predict('AMZN', AMZN_x_test, AMZN_y_test, model_GRU)
# Predict('GOOG', GOOG_x_test, GOOG_y_test, model_GRU)
# Predict('TSLA', TSLA_x_test, TSLA_y_test, model_GRU)
# Predict('AMZN', AMZN_x_test, AMZN_y_test, model_LSTM_self)
# Predict('GOOG', GOOG_x_test, GOOG_y_test, model_LSTM_self)
# Predict('TSLA', TSLA_x_test, TSLA_y_test, model_LSTM_self)

def predict_future_prices(model, last_known_data, steps=30):
    """
    使用模型预测未来股票价格。
    参数:
        model: 已训练的模型。
...
# 这里的`last_30_days_scaled`应是一个形状为(30, 1)的数组
last_30_days_scaled = AMZN_y_test[-30:]

# 调用预测函数
predicted_prices = predict_future_prices(model_LSTM, last_30_days_scaled)

# 反归一化预测结果
predicted_prices = scaler.inverse_transform(predicted_prices)

combined_array = np.concatenate((AMZN_y_test[-30:], predicted_prices), axis=0)
# 数据可视化
plt.rcParams['font.family'] = 'Microsoft YaHei'  # Microsoft YaHei, SimHei
plt.figure(figsize=(16, 8))
plt.plot(combined_array, label='预测价格')
plt.plot(AMZN_y_test[-30:], label='历史价格')
plt.xlabel('时间')
plt.ylabel('收盘价')
plt.legend()
plt.show()